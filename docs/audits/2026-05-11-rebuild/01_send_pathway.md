...
- **Evidence**: `grep DECISION_FILE C:/Foundry/Rexfinhub` returns exactly one writer (admin_reports.py:302). Live VPS shows `.preflight_token` and `.preflight_result.json` updated 18:30:48 today, but **no `.preflight_decision.json`** in `/home/jarvis/rexfinhub/data/`. Last 6 calendar weekdays of journald all show identical "ABORT… Dashboard click required" → "send_all: standing down" sequence.
