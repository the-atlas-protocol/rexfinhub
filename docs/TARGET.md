...
~~Phase 5 — Build `status_history` bi-temporal table~~ **STAGES 1-2 SCHEMA + BACKFILL SHIPPED 2026-05-19** (PR #32). 608 rows: 541 current-state + 67 historical 'effective' rows for Listed funds. Remaining: Stage 3 reconciler implementation with 3-source rule + 3-day dry-run + cutover. Designed in `DECISIONS/0008-status-history-bitemporal.md`.
...
~~Phase 6 — Replace 6 CSVs with `classification_override` table~~ **STAGES 1-2 SCHEMA + CSV MIGRATION SHIPPED 2026-05-19** (PR #32). 486 override rows migrated from 7 rule CSVs. assertion_run table created. Remaining: Stages 3-7 (resolver code, admin write surface, 25 assertions, timer move to 08:00 ET, drop CSVs). Designed in `DECISIONS/0009-classification-override-and-assertions.md`.
