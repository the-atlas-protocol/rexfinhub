...
| What | Command |
|------|---------|
| Start local server | `cd C:\Foundry\Rexfinhub; uvicorn webapp.main:app --reload --port 8000` |
| Run full pipeline (incremental) | `cd C:\Foundry\Rexfinhub; python run_daily.py` |
| Run pipeline only (no email) | `cd C:\Foundry\Rexfinhub; python -c "from etp_tracker.run_pipeline import run_pipeline; from etp_tracker.trusts import get_all_ciks, get_overrides; run_pipeline(ciks=list(get_all_ciks()), overrides=dict(get_overrides()), refresh_submissions=True, user_agent='REX-ETP-Tracker/2.0')"` |
| Force full reprocess | `cd C:\Foundry\Rexfinhub; python -c "from etp_tracker.run_pipeline import run_pipeline; from etp_tracker.trusts import get_all_ciks, get_overrides; run_pipeline(ciks=list(get_all_ciks()), overrides=dict(get_overrides()), refresh_submissions=True, user_agent='REX-ETP-Tracker/2.0', force_reprocess=True)"` |
| Sync pipeline data to DB | `cd C:\Foundry\Rexfinhub; python -c "from webapp.database import SessionLocal, init_db; from webapp.services.sync_service import seed_trusts, sync_all; from pathlib import Path; init_db(); db=SessionLocal(); seed_trusts(db); sync_all(db, Path('outputs')); db.close(); print('Done')"` |
| Generate screener PDF | `cd C:\Foundry\Rexfinhub; python screener/generate_report.py` |
| Run candidate evaluation | `cd C:\Foundry\Rexfinhub; python screener/generate_report.py evaluate SCCO BHP RIO` |
...
2. Save the Excel file as `C:\Foundry\Rexfinhub\data\SCREENER\data.xlsx`
...
```powershell
schtasks /create /tn "ETP_Filing_Tracker" /tr "python C:\Foundry\Rexfinhub\run_daily.py" /sc daily /st 08:00 /f
```
