...
- **Remaining (operator-gated)** — Track 5A reconciler `--apply` awaits Ryu's review of the 175/39 dry-run diff (decision D3); Track 5B pre-filing matcher is downstream of that; Track 6 edgartools migration is a multi-week effort with a mandatory 2-week dual-run; Track 0 `ADMIN_PASSWORD` rotation needs Render env access.
