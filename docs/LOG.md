...
- **Remaining** — Track 5B (pre-filing-product admin surface + filing matcher) is a new feature for a workflow not yet in use; the duplicate concern it targets is already structurally prevented by the Phase 4 canonical-identity model + the `identifier_xref_consistency` assertion. Track 6 (edgartools migration) is a multi-week effort with an ADR-0010-mandated 2-week dual-run — not completable in a single session; it is the live SEC-ingestion engine and warrants operator-supervised cutover. Track 0 (`ADMIN_PASSWORD` rotation) needs Render env access.
