...
- **Full-scale rebuild audit + CBOE Cloudflare-block diagnosis** (PR #81). Post-rebuild audit verified production end-to-end: all 19 PRs (#62-#80) merged; VPS on #79 (#80 docs-only); all 10 rebuild tables present; `capm_products` dropped; 25/25 assertions pass (08:00 run); `status_history` populated (783 rows, reconciled); all systemd timers healthy; Render `/admin/` returns 302 not 500 (the #79 fix holds). Two findings:
  - **CBOE is not a cookie problem.** `rexfinhub-cboe.service` 403s because CBOE's Cloudflare WAF blocks the VPS's datacenter egress IP at the edge — proven by probes: the VPS gets `403 "Sorry, you have been blocked"` over both IPv4 and IPv6, with or without a cookie, while the same request from a residential IP passes Cloudflare (`302`). `scanner.py` / `live.py` mislabelled every 403 as `refresh CBOE_SESSION_COOKIE`, sending the operator into futile cookie rotations. Fixed: `_is_cloudflare_block()` detects the WAF block page; `CloudflareBlockError` (subclass of `AuthError`) carries an accurate message; `auth_health()` gained a `blocked` state; `/filings/symbols` renders an amber "blocked at the network edge" banner. Unblocking the scanner needs an egress-IP change (proxy) or CBOE allowlisting the VPS — an operator decision; the 475k-ticker sweep at concurrency 300 is the likely trigger and should be throttled.
  - **`rex_products.underlier` column was never physically dropped on production.** Track 4b (#66) dropped it in dev behind a Gate-C proof, but `scripts/drop_rex_underlier_column.py` was never run on the VPS. Harmless — all 541 values are NULL and the ORM `hybrid_property` ignores the physical column — but the REBUILD-STATE doc's "physically retired" claim is corrected.
- **CBOE scan shrunk to a polite footprint** (PR #81). The nightly `rexfinhub-cboe.service` ran `--tier full` — all 475,254 symbols at concurrency 300, an unmistakable flood and the likely trigger for the Cloudflare WAF block. Fixed: the service now runs `--tier daily --concurrency 4`. The `daily` tier was also rebuilt — it previously folded in `previously_available` (every known-available ticker), which after a full sweep meant ~450k symbols, no smaller than `full`. It is now 1-3 letter (18,278) + a rolling 50k 4-letter slice + tickers state-changed in the last 30 days ≈ 68k/night: **7x fewer requests, ~75x lower burst concurrency**, full 4-letter coverage still completing on a ~9-night rolling cycle. This does not by itself lift the current IP block — it stops re-provoking it. Activating the new unit needs `systemctl daemon-reload` on the VPS.
