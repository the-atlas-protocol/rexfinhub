...
def _capm_seed_if_empty():
    """Seed capm_trust_aps from the bundled CSV if empty.

    Background: the Trust & APs data is curated locally (Excel import on
    Ryu's desktop) and is NOT regenerated by the VPS daily pipeline. When
    the VPS uploads its DB to Render, the freshly-swapped DB has zero
    capm_trust_aps rows — wiping the Trust & APs tab until Ryu manually
    re-uploads from desktop. Auto-seeding from a checked-in CSV makes
    the page survive any DB swap (VPS upload, deploy reset, etc.).

    (capm_products is retired — Track 4a — so it is no longer seeded.)
