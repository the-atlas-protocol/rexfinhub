...
def load_recent_competitor_filings():
    """Last 60d non-REX 485APOS, strict L&I, exclude 3X+, dedup by series."""
...
def load_trex_2x_long_pipeline():
    """ONLY T-REX 2X Long products (the leveraged-long T-REX series). Excludes
    REX IncomeMax / REX Growth / sector-basket products. Scored from
    li_engine_daily (the full v1.0.2 universe).
    """
    df = _df("""SELECT ticker, name AS fund_name, direction, status,
                ...
                WHERE (name LIKE 'T-REX 2X Long%' OR name LIKE 'T-REX 2X LONG%')
                  AND status IN ('Filed','Effective','Listed','Target List')
...
def load_launch_anyway(rex_pos: dict, single_stocks: set):
    """1-2 competitor, REX not in, top product > $300M AUM. Enriched with first-launch date."""
    df = _df("""SELECT map_li_underlier AS u, map_li_direction AS d,
                  is_rex, COALESCE(issuer_nickname, issuer) AS issuer,
    ...
    for u, g in df[df["is_long"]].groupby("u_clean"):
        ...
        # Find earliest inception (first launch date)
    ...
    return pd.DataFrame(rows).sort_values("top_aum", ascending=False).head(20) if rows else pd.DataFrame()
...
def load_flow_score_backtest():
    """NEW: For each L&I underlier, compute 3mo AUM change (proxy for flow) and the
    underlier's current v1.0.2 score. Bucket analysis answers: does score predict flow?
    """
    # Current AUM by underlier (sum across all L&I products on that underlier)
...
def build():
    ...
    flow_top, flow_buckets = load_flow_score_backtest()
...
{_table_header(['Score Bucket','Underliers','Avg 3mo Flow','Median 3mo Flow','% with Inflow'])}
{bucket_rows}</table>"""
        ...
        flow_table = f"""<div style="font-size:11px;color:{GRAY};margin-top:14px;margin-bottom:4px;font-weight:600;">Top 20 Underliers by Absolute 3-Month Flow</div>
<table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
{_table_header(['Underlier','Current Total AUM','3mo Net Flow','# Products','REX Count','v1.0.2 Score'])}
{flow_rows}</table>"""
        ...
        track_html = f"""
<div style="font-size:12px;color:{NAVY};line-height:1.7;margin-bottom:8px;">
  <strong>Concurrent backtest:</strong> For every L&amp;I underlier with at least one active product, we compute the 3-month net AUM change (a proxy for flow) and join it to today's v1.0.2 score. If our scoring system is working, high-scored underliers should have higher net flow than low-scored ones.
</div>
{bucket_table}
{flow_table}
{recs_html}
...
<div style="font-size:12px;color:#1a1a2e;line-height:1.7;">
  ...
  <p><strong>Track record — concurrent backtest:</strong> For each L&amp;I underlier with at least one active product, we sum today's AUM across all L&amp;I products on that underlier, subtract the same sum from 3 months ago (<code>mkt_time_series.aum_value</code> with <code>months_ago=3</code>), and compare to the underlier's v1.0.2 score. If the scoring works, HIGH-score buckets should show larger / more positive average flows than LOW-score buckets. This is concurrent validity, not a forward test — the forward test (recommendation_history hit-rate) needs ~10 weeks of accrued recommendations before it's statistically meaningful.</p>
  ...
  <p style="margin-top:10px;font-size:11px;color:{GRAY};"><strong>Changes since v1.0.1:</strong> race timing removed from scoring buckets (now its own section); tier bands (LAUNCH/FILE/WATCH) removed; methodology embedded; track record (flow backtest + forward hit-rate) added; delisting watch added; pipeline restricted to T-REX 2X Long; recent filings tightened to 60d ≤2X.</p>
</div>
"""
    ...
    if flow_buckets:
        ...
        if abs(h) > 1 or abs(l) > 1:
            verdict = "scoring directionally tracks flows" if h > l else "scoring does NOT track flows (HIGH avg <= LOW avg)"
            bucket_signal = f"3mo backtest: HIGH bucket avg flow {_fmt_flow_m(h)} vs LOW bucket {_fmt_flow_m(l)} — {verdict}"
...
<tr><td style="background:{NAVY};padding:22px 30px;">
  ...
  <div style="color:#9bb1cc;font-size:11px;font-weight:500;letter-spacing:1px;text-transform:uppercase;margin-top:6px;">
    v1.0.2 · Imminent · New Comp Filings · T-REX 2X Pipeline · Mega · Inverse · Launch-Anyway · Foreign · Pre-IPO · Delisting · Backtest · Methodology
  </div>
</td></tr>
<tr><td style="padding:12px 30px 0;">
  <div style="font-size:12px;color:{GRAY};line-height:1.7;">
    ...
    <strong style="color:{NAVY};">T-REX 2X Long pipeline:</strong> <span style="color:{TEAL};font-weight:700;">{n_pipeline} products</span> ·
...
{_section_header('1 · Imminent Launches — Competitor L&I PEND', RED, 'Competitor L&I PEND products with future or unspecified inception dates. Historical no-launch entries excluded.')}
<tr><td style="padding:6px 30px 8px;">{imminent_html}</td></tr>

{_section_header('Recent Competitor L&I Filings — New Activity (last 60 days, ≤2X only)', ORANGE, 'Non-REX 485APOS matching strict L&I patterns. 3X+ products excluded. Deduped by series name.')}
<tr><td style="padding:6px 30px 8px;">{recent_html}</td></tr>

{_section_header('2 · Our Pipeline — T-REX 2X Long, Top 20 by Underlier Score', BLUE, 'Restricted to T-REX 2X Long products only (long-leveraged single-stock series). Underlier score from li_engine_daily (v1.0.2). Status: Filed or Effective.')}
...
{_section_header('5 · Launch Anyway — 1-2 Competitor, REX Not In (Top 20)', BLUE, 'Single-stock underliers with 1-2 competitor longs averaging > $300M AUM, REX has no position. First-launch date + age included to gauge product seasoning.')}
...
{_section_header('7 · Pre-IPO & Recently Priced (IPO Watchlist)', TEAL, f'YAML-backed valuations (last refresh {yaml_refresh_note}). Rows aged > 60d flagged in Data Age. L&I filer race cross-ref shown.')}
<tr><td style="padding:6px 30px 8px;">{ipo_html}</td></tr>

{_section_header("8 · Delisting Watch — REX L&I < $15M @ 6mo", RED, "REX-only. ACTV products with AUM under $15M and age >= 5.5 months. Rule of thumb: funds that have not crossed $15M by month 6 historically do not graduate.")}
<tr><td style="padding:6px 30px 8px;">{del_html}</td></tr>

{_section_header('9 · Track Record — Scoring vs Actual L&I Flows (3mo)', GREEN, 'For each L&I underlier, 3-month net AUM change vs our v1.0.2 score. Concurrent validity check: are high-scored underliers seeing more flow?')}
