...
<table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
{_table_header(['Underlier','Current Total AUM','3mo Net Flow','# Products','REX Count','v1.0.2 Score'], aligns=['left','right','right','center','center','right'])}
{flow_rows}</table>"""
...
<div style="font-size:12px;color:{NAVY};line-height:1.7;margin-bottom:10px;background:{LIGHT};padding:12px 14px;border-radius:6px;border-left:4px solid {GREEN};">
  ...
  <div style="font-size:11px;color:{GRAY};margin-bottom:6px;">For every leveraged-&-inverse underlier with at least one active product, we sum today's total AUM and subtract the same total from 3 months ago to get net flow. We then join the underlier's current v1.0.2 score to see whether high-score underliers actually attract money.</div>
...
{_section_header('2 · Our Pipeline — T-REX 2X Long Filed, Not Yet Launched (Top 20)', BLUE, 'T-REX 2X Long products in Filed / Under Consideration / Target List status — what we have on file but have not yet shipped to market. Underlier score from li_engine_daily (v1.0.2). Pre-IPO targets (Anthropic / SpaceX / etc.) appear with no score since their underlier is private.')}
<tr><td style="padding:6px 30px 8px;">{pipeline_html}</td></tr>

{_section_header('3 · Whitespace Ranking — Top 100 Underliers With No Live L&I Product', NAVY, 'From the curated whitespace pool (1,417 tickers), ranked by v1.0.2 composite score. Filter: no active L&I product on the underlier yet. REX/Comp filing flags show whether anyone has filed.')}
...
{_section_header("8 · T-REX Delisting Watch — Live Products < $15M @ 6mo+", RED, "T-REX brand only (excludes MicroSectors). ACTV products with AUM under $15M and age >= 5.5 months. Rule of thumb: funds that have not crossed $15M by month 6 historically do not graduate, so these are review candidates.")}
<tr><td style="padding:6px 30px 8px;">{del_html}</td></tr>

{_section_header('9 · Scoring Track Record — Does Our Score Predict Real Flows?', GREEN, 'Concurrent backtest. Group every L&I underlier by its current v1.0.2 score band (HIGH / MED / LOW) and compare the average 3-month net AUM change in each band. If our score works, the HIGH band should see more flow than the LOW band.')}
